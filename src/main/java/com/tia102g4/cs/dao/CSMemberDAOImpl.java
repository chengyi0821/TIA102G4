...
public class CSMemberDAOImpl implements CSDAO {
	...
	@Override
	public void update(Long csId, Integer replyHeading, String replyContent,Timestamp replyTime ,Boolean replyStatus, Long adminId) {
	...
	public List<CustomerService> getByCompositeQuery(Map<String, String> map) {
		...
		for (Map.Entry<String, String> row : map.entrySet()) {
			...
			if ("searchQuery".equals(row.getKey())) {
				Join<CustomerService, Member> memberJoin = root.join("member");
				predicates.add(builder.like(memberJoin.get("name"), "%" + row.getValue() + "%"));
