...
<link
	...
	rel="stylesheet" />
<link
	href="<%=request.getContextPath()%>/frontstage/backend/admin/css/admin.css"
	rel="stylesheet" />
</head>
<body>
	<!-- Topbar 開始 -->
				...
				<div class="collapse navbar-collapse justify-content-between"
					...
					<div class="navbar-nav ml-auto py-0">
						...
						<div class="orderblock">
							...
							<ul class="orderlist">
								<li><a style="color: black;"
									href="<%=request.getContextPath()%>/frontstage/memberFrontend/member/memberindex.jsp">會員列表</a></li>
								<li><a style="color: black;"
	...
	<div id="main-content2">
		...
		<form action="<%=request.getContextPath()%>/admin/admin.do"
			...
			<table style="width: 100%; text-align: center;">
				...
				<c:forEach var="admin" items="${adminList}">
					<tr>
						...
						<td>
							<form
								action="<%=request.getContextPath()%>/admin/admin.do?action=delete"
								method="post" style="display: inline">
								<input type="hidden" name="adminId" value="${admin.adminId}">
								<input type="submit" value="刪除"
									onclick="return confirm('確定要刪除這個管理員嗎？');">
							</form> <a
							href="<%=request.getContextPath() %>/admin/admin.do?action=getById&adminId=${admin.adminId}">修改</a>
						</td>
					</tr>
				</c:forEach>
			...
			<c:if test="${currentPage > 1}">
				<a
					href="<%=request.getContextPath()%>/admin/admin.do?action=getAll&page=1">至第一頁</a>&nbsp;
            </c:if>
			<c:if test="${currentPage - 1 > 0}">
				<a
					href="<%=request.getContextPath() %>/admin/admin.do?action=getAll&page=${currentPage - 1}">上一頁</a>&nbsp;
            </c:if>
			<c:if test="${currentPage + 1 <= adminPageQty}">
				<a
					href="<%=request.getContextPath() %>/admin/admin.do?action=getAll&page=${currentPage + 1}">下一頁</a>&nbsp;
            </c:if>
			<c:if test="${currentPage < adminPageQty}">
				<a
					href="<%=request.getContextPath()%>/admin/admin.do?action=getAll&page=${adminPageQty}">至最後一頁</a>&nbsp;
            </c:if>
			<br> <a
				href="<%=request.getContextPath()%>/frontstage/backend/admin/adminindex.jsp">回首頁</a>
		</form>
	</div>
