		...
		<div class="main_content">
			...
			<div class="search_quickly">
				...
				<button class="btn_search">
					<a
						href="<%=request.getContextPath()%>/myorder/myorder.do?action=getAll"
						class="allorder">所有訂單</a>
				</button>
				<button class="btn_search"
					style="background-color: #fdc001; border: #fdc001;">
					<a
						href="<%=request.getContextPath()%>/myorder/myorder.do?action=getOrderStatus1"
						class="allorder">預約訂單</a>
				</button>
				<button class="btn_search">
					<a class="allorder"
						href="<%=request.getContextPath()%>/myorder/myorder.do?action=getOrderStatus2"
						role="button">被取消訂單</a>
				</button>
				<button class="btn_search">
					<a class="allorder"
						href="<%=request.getContextPath()%>/myorder/myorder.do?action=getOrderStatus3"
						role="button">已完成訂單</a>
				</button>
			</div>
			...
			<div class="form">
				<div class="pagination-info1">
					<form action="<%=request.getContextPath()%>/myorder/myorder.do"
						method="post">
						<input type="hidden" name="action" value="getOrderStatus1">
					...
					<div class="pagination">
						<c:if test="${currentPage > 1}">
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath()%>/myorder/myorder.do?action=getOrderStatus1&page=1">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
							...
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/myorder.do?action=getOrderStatus1&page=${currentPage - 1}">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
							...
							<c:choose>
								<c:when
									...
									<c:if test="${page != currentPage}">
										<button class="page_number">
											<a style="color: black;"
												href="<%=request.getContextPath() %>/myorder/myorder.do?action=getOrderStatus1&page=${page}">${page}</a>
										</button>
									</c:if>
								</c:when>
							</c:choose>
						...
						<c:if test="${currentPage < totalPages}">
							...
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/myorder.do?action=getOrderStatus1&page=${totalPages}">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            button.addEventListener('click', function (event) {
                ...
                if (!orderId && !memberName && !reserDate) {
                    ...
                    window.location.href = '${pageContext.request.contextPath}/myorder/myorder.do?action=getOrderStatus1';
                }
            });
        });
//        ==================================================================================================================================== 
        ...
        function showConfirmationMessage() {
            ...
            form.method = "post";
            form.action = "${pageContext.request.contextPath}/myorder/myorder.do";
            
            var actionInput = document.createElement("input");
            actionInput.type = "hidden";
            actionInput.name = "action";
            actionInput.value = "updateOrderStatus2";
...
//         ========================================================================================================================================
	  function showCancelConfirmation1(orderId) {
        ...
        function showConfirmationMessage1() {
            ...
            actionInput.type = "hidden";
            actionInput.name = "action";
            actionInput.value = "updateOrderStatus3";
            form.appendChild(actionInput);
