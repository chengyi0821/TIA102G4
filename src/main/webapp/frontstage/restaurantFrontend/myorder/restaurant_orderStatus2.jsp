		...
		<div class="main_content">
			...
			<div class="form">
				<div class="pagination-info1">
					<form
						action="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getOrderStatus2Rest"
						method="get">
						<input type="hidden" name="action" value="getAll"> 跳轉到第 <input
							class="search_page" type="number" name="page" min="1"
					...
					<div class="pagination">
						<c:if test="${currentPage > 1}">
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getOrderStatus2Rest&page=1"><svg
							...
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getgetOrderStatus2Rest&page=${currentPage - 1}"><svg
										xmlns="http://www.w3.org/2000/svg" width="16" height="16"
							...
							<c:choose>
								<c:when
									...
									<c:if test="${page != currentPage}">
										<button class="page_number">
											<a style="color: black;"
												href="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getOrderStatus2Rest&page=${page}">${page}</a>
										</button>
									</c:if>
								</c:when>
						...
						<c:if test="${currentPage < totalPages}">
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getOrderStatus2Rest&page=${currentPage + 1}"><svg
										xmlns="http://www.w3.org/2000/svg" width="16" height="16"
										fill="currentColor" class="bi bi-chevron-right"
										viewBox="0 0 16 16">
  							<path fill-rule="evenodd"
							...
							<button class="page_number">
								<a style="color: black;"
									href="<%=request.getContextPath() %>/myorder/restmyorder.do?action=getOrderStatus2Rest&page=${totalPages}"><svg
										xmlns="http://www.w3.org/2000/svg" width="16" height="16"
									...
									.addEventListener(
											...
											function(event) {
												...
												if (!orderId && !memberName
													...
													window.location.href = '${pageContext.request.contextPath}/myorder/restmyorder.do?action=getOrderStatus2Rest';
												}
											});
						});
	</script>
